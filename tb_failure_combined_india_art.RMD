...
```{r}
#assays(indata)
#indata <- mkAssay(indata,  log = TRUE)


#assay_type = "log_counts"
...
plot(g)
#ggsave("tb_fail_umap_log_counts_pre-batch-corrected.png", width=7, height=7)
```
...
names(assays(indata)) <- c("counts", "combatseq", "log_counts", "cpm", "log_cpm")
...
ggsave("tb_fail_umap_log_counts_post-batch-corrected.png", width=7, height=7)
```
...
```{r base-pca}
sctkplotPCA <- function (inSCE, colorBy = NULL, shape = NULL, pcX = "PC1", 
  pcY = "PC2", reducedDimName = "PCA", runPCA = FALSE, useAssay = "logcounts") 
{
  if (!(reducedDimName %in% names(SingleCellExperiment::reducedDims(inSCE)))) {
    if (runPCA) {
      inSCE <- scaterPCA(inSCE, useAssay = useAssay, reducedDimName = reducedDimName)
    }
    else {
      stop(reducedDimName, " dimension not found. Run scaterPCA() or set runPCA to TRUE.")
    }
  }
  ...
  pcYlab <- pcY
  if (!is.null(colorBy)) {
    pcaDf$color <- SingleCellExperiment::colData(inSCE)[, 
      colorBy]
  }
  if (!is.null(shape)) {
    pcaDf$shape <- factor(SingleCellExperiment::colData(inSCE)[, 
      shape])
  }
  pcaDf$Sample <- colnames(inSCE)
  g <- ggplot2::ggplot(pcaDf, ggplot2::aes_string(pcX, pcY, 
    label = "Sample")) + ggplot2::geom_point() + ggplot2::labs(x = pcXlab, 
    y = pcYlab)
  if (!is.null(colorBy)) {
    g <- g + ggplot2::aes_string(color = "color") + ggplot2::labs(color = colorBy)
  }
  if (!is.null(shape)) {
    g <- g + ggplot2::aes_string(shape = "shape") + ggplot2::labs(shape = shape)
  }
  return(g)
}
...
#write.csv(limmaResNut[1:3000,], "IPA_TB_Failure_baseline.csv")
...
#library(Cairo)
svg("plots/differential_expression_heatmaps/baseline_top1k.svg", width=7, height=7 )
g
dev.off()
#ggsave("plots/differential_expression_heatmaps/baseline_top1k.svg", width=7, height=7, plot=g, device = "svg")
```
...
TBsignatures$vanvalkenburg_6_bl_xmdx <- NULL
...
colData(ssgsea_res_select)$visit_type <- gsub("failure", "Failure", colData(ssgsea_res_select)$visit_type)
colData(ssgsea_res_select.bl)$visit_type <- gsub("failure", "Failure", colData(ssgsea_res_select.bl)$visit_type)
colData(ssgsea_res_select.m2)$visit_type <- gsub("failure", "Failure", colData(ssgsea_res_select.m2)$visit_type)

colData(ssgsea_res_select)$subjtype <- gsub("failure", "Failure", colData(ssgsea_res_select)$subjtype)
colData(ssgsea_res_select.bl)$subjtype <- gsub("failure", "Failure", colData(ssgsea_res_select.bl)$subjtype)
colData(ssgsea_res_select.m2)$subjtype <- gsub("failure", "Failure", colData(ssgsea_res_select.m2)$subjtype)


#png("ssgsea_selectsig_boxplot_bl.png", width = 6.5, pointsize = 8)
...
df <- tableAUC(ssgsea_res_select.bl,
         ...
         pb.show = FALSE,
         output = "data.frame")
write.csv(df, file.path(path, "tbsigprofiler_selectsigs_AUC_boxplots_table_bl.csv"))
...
write.csv(df, file.path(path, "tbsigprofiler_selectsigs_AUC_boxplots_table_m2.csv"))
...
compareBoxplots(ssgsea_res_select.m2, annotationColName = "subjtype",
                ...
                pb.show = FALSE, fill.col = "blue",
                rotateLabels = TRUE)
ggsave(file.path(path,"tbsigprofiler_selectsigs_AUC_boxplots_m2.svg"), width = 7, height=7)
...
```{r base-gsva-boxplot-ind, results="asis"}
...
```{r base-gsva-sigplots-ind, results="asis"}
for (i in names(samp_tbsignatures)){
...
#### AUC Table {.tabset}
```{r base-gsva-auctable, message = FALSE}
set.seed(0)
tableAUC(gsva_res,
         annotationColName = "subjtype",
...
#### AUC Boxplots
```{r base-gsva-auc-boxplot, message = FALSE}
set.seed(0)
...
#write.csv(limmaResNut[1:3000,], "IPA_TB_Failure_month2.csv")
```
...
svg("plots/differential_expression_heatmaps/month2_top1k.svg", width=7, height=7 )
g
dev.off()
g
```
...
m <- lapply(l, unlist)
#m <- bind_cols(x)
n <- as.character(1:31380)
nnn <- paste("X", n, sep = "")

names(m) <- nnn

ssgsea_res <- runTBsigProfiler(indata_base, useAssay = "log_cpm", algorithm = "ssGSEA",
                               signatures = m, parallel.sz = 4)
```
...
# X31250 has the smallest pval and largest AUC
l2 <- m[["X31062"]]
...
df <- df[order(-df$AUC, -df$neg10xLog.P.value.),]
View(df)
...
#rownames.mat.         gene1         gene2         gene3 
#      "RNF10"       "RREB1"        "PFKL"        "NADK" 
#        gene4         gene5 
#     "PIK3CD"       "CALCR" 

#vanvalkenburg_6_bl <- c("AC008763.2", "AC106017.2", "BICD2", "AC016245.1", "BACH1-AS1", "ATP9A")

#TBsignatures$VanValkenburg_6 <- c("RNF10", "RREB1", "PFKL", "NADK", "PIK3CD", "CALCR")

#### also, instead of RNF10, LINC00472 or AL157384.1. Picked bc of protein coding
```
...
## New sig, all timepoints {.tabset}

```{r, eval=FALSE}
designMat <- model.matrix(~factor(subjtype) , data=colData(indata))
...
mat <- as.matrix(assay(indata, "log_cpm"))
bl_mat <- as.matrix(assay(indata, "log_cpm"))
sig_bl_mat <- t(bl_mat[rownames(dfx),])
...
tx_status = data.frame(indata$subjtype)
y <- data.frame()
...
#vanvalkenburg_6_all <- c("AC010889.2", "AC124014.1", "RN7SL288P", "PRR5-ARHGAP8", "WRAP73", "LRRFIP1")

#vanvalkenburg_6_m2 <- c()
...
# Genes from Protein biomarkers baseline
```{r}
genelist <- c("IL6", "IFNG", "IL13", "CXCL1", "CCL2", "CCL3", "CCL4", "CXCL8", "CXCL10",  "CX3CL1")


mat = as.matrix(assay(indata_base[genelist,],"log_cpm"))
mat = t(scale(t(mat)))
df <- as.data.frame(colData(indata_base))
df <- merge(df, t(mat), by = 'row.names', all=T)


indata_genes_bl <- SummarizedExperiment(assays=assays(indata_base), 
                                        colData = df)
genelist <- list("IL6", "IFNG", "IL13", "CXCL1", "CCL2", "CCL3", "CCL4", "CXCL8", "CXCL10",  "CX3CL1")

names(genelist) <- c("IL6", "IFNG", "IL13", "CXCL1", "CCL2", "CCL3", "CCL4", "CXCL8", "CXCL10",  "CX3CL1")

#png(filename = "Individual_serumprotein_genes.png")

signatureBoxplot(indata_genes_bl,
                 name = "Gene Boxplots",
                 signatureColNames = names(genelist),
                 annotationColName = "subjtype"
                 )
#dev.off()
...
# Genes from Protein biomarkers M2
```{r}
genelist <- c("IL6", "IFNG", "IL13", "CXCL1", "CCL2", "CCL3", "CCL4", "CXCL8", "CXCL10",  "CX3CL1")


mat = as.matrix(assay(indata_m2[genelist,],"log_cpm"))
mat = t(scale(t(mat)))
df <- as.data.frame(colData(indata_m2))
df <- merge(df, t(mat), by = 'row.names', all=T)


indata_genes_m2 <- SummarizedExperiment(assays=assays(indata_m2), 
                                        colData = df)
genelist <- list("IL6", "IFNG", "IL13", "CXCL1", "CCL2", "CCL3", "CCL4", "CXCL8", "CXCL10",  "CX3CL1")

names(genelist) <- c("IL6", "IFNG", "IL13", "CXCL1", "CCL2", "CCL3", "CCL4", "CXCL8", "CXCL10",  "CX3CL1")

#png(filename = "Individual_serumprotein_genes_m2.png")

signatureBoxplot(indata_genes_m2,
                 name = "Gene Boxplots",
                 signatureColNames = names(genelist),
                 annotationColName = "subjtype"
                 )
...
## TBSigProfiler Cancer Pathways signatures Baseline {.tabset}
```{r, message=FALSE, results='hide'}
TBsignatures$vanvalkenburg_6_bl_xmdx <- NULL
...
samp_tbsignatures <- prCancer_sigs
...
```{r base-ssgsea-cancer-boxplot-selectsigs}
select_tbsignatures <- list(TBsignatures$Thompson_FAIL_13,
                            TBsignatures$Tornheim_RES_25,
                            TBsignatures$Tornheim_71, 
                            TBsignatures$Bloom_RES_558,
...
# ssgsea_res_select <- runTBsigProfiler(indata_base, useAssay = "log_cpm", algorithm = "ssGSEA",
#                                signatures = select_tbsignatures, parallel.sz = 4)
# 
# svg("ssgsea_selectsig_boxplot_bl.svg", width = 6.5, pointsize = 8)
# signatureBoxplot(ssgsea_res_select, name="ssGSEA", signatureColNames = names(select_tbsignatures),
#                  annotationColName = c("subjtype"), scale = TRUE) #rotateLabels = TRUE,
# dev.off()
```
...
```{r base-gsva-cancer-boxplot-ind, results="asis"}
for (i in names(samp_tbsignatures)){
...
#### Signature plots {.tabset}
```{r base-gsva-cancer-sigplots-ind, results="asis"}
for (i in names(samp_tbsignatures)){
...
#### AUC Table {.tabset}
```{r base-gsva-cancer-auctable, message = FALSE}
set.seed(0)
tableAUC(gsva_res,
         annotationColName = "subjtype",
         signatureColNames = names(samp_tbsignatures),
...
#### AUC Boxplots
```{r base-gsva-cancer-auc-boxplot, message = FALSE}
set.seed(0)
compareBoxplots(gsva_res, annotationColName = "subjtype",
                signatureColNames = names(samp_tbsignatures),
                pb.show = FALSE, fill.col = "blue",
...
```{r base-ssgsea-cancer-boxplot-ind, results="asis"}
for (i in names(samp_tbsignatures)){
...
#### Signature plots {.tabset}
```{r base-ssgsea-cancer-sigplots, results="asis"}
for (i in names(samp_tbsignatures)){
...
#### AUC Table
```{r base-ssgsea-cancer-auctable, message = FALSE}
set.seed(0)
tableAUC(ssgsea_res,
         annotationColName = "subjtype",
         signatureColNames = names(samp_tbsignatures),
...
#### AUC Boxplots {.tabset}
```{r base-ssgsea-cancer-auc-boxplot, message = FALSE}
#svg(filename = "base-ssgsea-cancer-auc-boxplot.svg", pointsize = 8, width = 6.5, height = 3.8) 
set.seed(0)
compareBoxplots(ssgsea_res, annotationColName = "subjtype",
                signatureColNames = names(samp_tbsignatures),
                pb.show = FALSE, fill.col = "blue",
...
#### ROC plots
```{r base-ssgsea-cancer-roc, message = FALSE, fig.height = 9, fig.width = 12}
signatureROCplot_CI(inputData = ssgsea_res,
                   signatureColNames = names(samp_tbsignatures),
                   annotationColName = "subjtype")
...
```{r base-ssgsea-cancer-roc-ind, results = 'asis', message = FALSE}
for (i in names(samp_tbsignatures)){
...
```{r base-plage-cancer-cancer-heatmap}
signatureHeatmap(plage_res, name="PLAGE", signatureColNames = names(samp_tbsignatures),
                 annotationColNames = c("subjtype"),
                 showColumnNames = FALSE,
                 split_heatmap='none')
...
```{r base-plage-cancer-cancer-boxplot}
signatureBoxplot(plage_res, name="PLAGE", signatureColNames = names(samp_tbsignatures),
                 annotationColName = c("subjtype"))# , rotateLabels = TRUE)
```
...
```{r base-plage-cancer-cancer-boxplot-ind, results="asis"}
for (i in names(samp_tbsignatures)){
...
#### Signature plots {.tabset}
```{r base-plage-cancer-cancer-sigplots, results="asis"}
for (i in names(samp_tbsignatures)){
...
#### AUC Table {.tabset}
```{r base-plage-cancer-cancer-auctable, message = FALSE}
set.seed(0)
tableAUC(plage_res,
         annotationColName = "subjtype",
         signatureColNames = names(samp_tbsignatures),
...
#### AUC Boxplots
```{r base-plage-cancer-cancer-auc-boxplot, message = FALSE}
set.seed(0)
compareBoxplots(plage_res, annotationColName = "subjtype",
                signatureColNames = names(samp_tbsignatures),
                pb.show = FALSE, fill.col = "blue",
...
#### ROC plots
```{r base-plage-cancer-cancer-roc, message = FALSE, fig.height = 9, fig.width = 12}
signatureROCplot_CI(inputData = plage_res,
                   signatureColNames = names(samp_tbsignatures),
                   annotationColName = "subjtype")
...
```{r base-plage-cancer-cancer-roc-ind, results = 'asis', message = FALSE}
for (i in names(samp_tbsignatures)){
...
## TBSigProfiler Cancer Pathways 2 months {.tabset}
```{r, message=FALSE, results='hide'}
...
```{r M2-gsva-cancer-boxplots-ind, results="asis"}
for (i in names(samp_tbsignatures)){
...
#### Signature plots {.tabset}
```{r M2-gsva-cancer-sigplots-ind, results="asis"}
for (i in names(samp_tbsignatures)){
...
#### AUC Table {.tabset}
```{r M2-gsva-cancer-auctable, message = FALSE}
set.seed(0)
tableAUC(gsva_res,
         annotationColName = "subjtype",
         signatureColNames = names(samp_tbsignatures),
...
#### AUC Boxplots
```{r M2-gsva-cancer-auc-boxplot, message = FALSE}
set.seed(0)
compareBoxplots(gsva_res, annotationColName = "subjtype",
                signatureColNames = names(samp_tbsignatures),
                pb.show = FALSE, fill.col = "blue",
...
#### ROC plots
```{r M2-gsva-cancer-roc, message = FALSE, fig.height = 9, fig.width = 12}
signatureROCplot_CI(inputData = gsva_res,
                   signatureColNames = names(samp_tbsignatures),
                   annotationColName = "subjtype")
...
```{r M2-gsva-cancer-roc-ind, results = 'asis', message = FALSE}
for (i in names(samp_tbsignatures)){
...
```{r M2-ssgsea-cancer-boxplot-ind, results="asis"}
for (i in names(samp_tbsignatures)){
...
#### Signature plots {.tabset}
```{r M2-ssgsea-cancer-sigplots-ind, results="asis"}
for (i in names(samp_tbsignatures)){
...
#### AUC Table
```{r M2-ssgsea-cancer-auctable, message = FALSE}
set.seed(0)
tableAUC(ssgsea_res,
         annotationColName = "subjtype",
         signatureColNames = names(samp_tbsignatures),
...
#### AUC Boxplots {.tabset}
```{r M2-ssgsea-cancer-auc-boxplot, message = FALSE}

#svg(filename = "M2-ssgsea-cancer-auc-boxplot.svg", pointsize = 8, width = 6.5, height = 3.8) 
set.seed(0)
compareBoxplots(ssgsea_res, annotationColName = "subjtype",
                signatureColNames = names(samp_tbsignatures),
                pb.show = FALSE, fill.col = "blue",
...
#### ROC plots
```{r M2-ssgsea-cancer-roc, message = FALSE, fig.height = 9, fig.width = 12}
signatureROCplot_CI(inputData = ssgsea_res,
                   signatureColNames = names(samp_tbsignatures),
                   annotationColName = "subjtype")
...
```{r M2-ssgsea-cancer-roc-ind, results = 'asis', message = FALSE}
for (i in names(samp_tbsignatures)){
...
```{r M2-plage-cancer-cancer-heatmap}
signatureHeatmap(plage_res, name="PLAGE", signatureColNames = names(samp_tbsignatures),
                 annotationColNames = c("subjtype"),
                 showColumnNames = FALSE,
                 split_heatmap='none')
...
```{r M2-plage-cancer-cancer-boxplot}
signatureBoxplot(plage_res, name="PLAGE", signatureColNames = names(samp_tbsignatures),
                 annotationColName = c("subjtype"))# , rotateLabels = TRUE)
```
...
```{r M2-plage-cancer-cancer-boxplot-ind, results="asis"}
for (i in names(samp_tbsignatures)){
...
#### Signature plots {.tabset}
```{r M2-plage-cancer-cancer-sigplots-ind, results="asis"}
for (i in names(samp_tbsignatures)){
...
#### AUC Table {.tabset}
```{r M2-plage-cancer-cancer-auctable, message = FALSE}
set.seed(0)
tableAUC(plage_res,
         annotationColName = "subjtype",
         signatureColNames = names(samp_tbsignatures),
...
#### AUC Boxplots
```{r M2-plage-cancer-cancer-auc-boxplot, message = FALSE}
set.seed(0)
compareBoxplots(plage_res, annotationColName = "subjtype",
                signatureColNames = names(samp_tbsignatures),
                pb.show = FALSE, fill.col = "blue",
...
#### ROC plots
```{r M2-plage-cancer-cancer-roc, message = FALSE, fig.height = 9, fig.width = 12}
signatureROCplot_CI(inputData = plage_res,
                   signatureColNames = names(samp_tbsignatures),
                   annotationColName = "subjtype")
...
```{r M2-plage-cancer-cancer-roc-ind, results = 'asis', message = FALSE}
for (i in names(samp_tbsignatures)){
...
```{r all-gsva-cancer-boxplot-ind, results="asis"}
for (i in names(samp_tbsignatures)){
...
#### Signature plots {.tabset}
```{r all-gsva-cancer-sigplots-ind, results="asis"}
for (i in names(samp_tbsignatures)){
...
```{r all-ssgsea-cancer-boxplots-ind, results="asis"}
for (i in names(samp_tbsignatures)){
...
#### Signature plots {.tabset}
```{r all-ssgsea-cancer-sigplots-ind, results="asis"}
for (i in names(samp_tbsignatures)){
...
```{r all-plage-cancer-boxplot-ind1, results="asis"}
for (i in names(samp_tbsignatures)){
...
#### Signature plots {.tabset}
```{r all-plage-cancer-sigplots-ind1, results="asis"}
for (i in names(samp_tbsignatures)){
